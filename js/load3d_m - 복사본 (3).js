...
controls.panSpeed = 1.8;            // [수정] 패닝이 너무 튀지 않게 적정값 조절
...
// --- 타겟 가이드 생성 ---   활성화 하려면 아래 코드 "3. 가이드 오브젝트(스피어) 위치 동기화" 이거도 켜줘야 함
...
// 2. [수정] 타겟의 Y와 Z값을 원하는 값으로 강제 고정
    const fixedY = 0.4;   // 바닥 높이 고정
    const fixedZ = 0.02; // 처음 설정한 Z값(0.02)으로 고정

    if (controls.target.y !== fixedY || controls.target.z !== fixedZ) {
        controls.target.y = fixedY;
        controls.target.z = fixedZ;
    }

    // 3. 가이드 오브젝트(스피어) 위치 동기화
    // if (targetGuide) {
